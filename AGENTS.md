...
- Use `xhs.cmd` as the only device-operation entry point. Do not call internal scripts, raw ADB, private APIs, or the Xiaowei WebSocket directly for device actions.
...
- Do not claim that a command, action, CPA role, or concurrent mode is available until current `xhs.cmd` help, tests, and preflight evidence prove it is implemented and enabled.
...
2. `feed_read_only`: the existing V1.1 foreground batch lane for one or two explicit machines; it continues to reject every interaction field.
...
`supervised_composite_v1` is the primary customization lane. It may run only after its compiler, approval, executor, and safety capabilities are implemented and verified. The first capability profile may support only two machines because that is the currently proven implementation capacity; later profiles may raise concurrency after tests and supervised acceptance without changing this governance rule. Until the composite runtime exists, the existing single-machine `trusted-10` command remains the only automated like/favorite acceptance path.
...
- `prepare` is a separate read-only `xhs.cmd` operation. It performs fresh inventory and only the capability checks required by the requested actions for the explicit machine list, writes `inventorySnapshotHash` and `capabilitySnapshotHash`, and performs no App/UI navigation. Read per-device interaction authorization only when the plan contains the corresponding like/favorite action; an unrelated input or interaction capability must not block a read-only plan.
...
- `feed.open_visible` selects from freshly verified semantic card nodes in stable visual order. A seeded ordinal chooses within the plan's finite `candidateCap`, which must fit the current capability profile; the initial profile may use four visible candidates. If the ordinal is unavailable, use only the approved bounded fallback; never click a random coordinate or silently choose another engagement target.
...
- The existing `feed batch` V1.1 remains read-only. Do not remove its interaction rejection or use it as the state-changing executor.
