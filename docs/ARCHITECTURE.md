...
3. **编排层**：`research-core.mjs` 继续负责只读研究；独立的 `supervised_composite_v1` 编排器负责显式有限设备清单、能力档案约束的 `maxParallel`、全员就绪屏障、父 lease、共享动作预算和单调全局熔断。设备数和并发数来自批准任务并由当前能力证据校验，仓库不设置永久数字上限。两个模式不共用宽化后的执行入口。
...
- `supervised_composite_v1` 使用显式有限设备清单、能力档案约束的 `maxParallel`、已批准的 all-ready/ready-subset 启动策略、一个共享 GO 和 lease-backed 执行槽 ledger；GO 只开启调度，worker 必须持 parent 授权票据及当前执行槽才可运行。请求的设备数和并发数必须有对应压力与故障验收证据；不发现、不重分配、不替换设备、互动目标或不可转移的动作预算槽。
