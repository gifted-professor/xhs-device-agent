...
这三条命令描述正式复合执行器的离线审阅、在线准备和哈希完整性绑定。用户已经明确请求任务时，Agent 可以连续完成准备并提交生成的 `--confirm-plan-hash`，不要求用户再次确认同一内容。多个 `--machine`、同一序号点赞加收藏以及任务指定的 `--max-parallel` 都会原样进入统一计划。普通命名 HTTP 或原子点赞/收藏不必进入此流程。
...
当前请求明确包含的登录、身份验证、支付、权限、互动或账号状态动作可以继续，不另加确认。设备、账号或目标发生漂移时重新观察和绑定；状态无法验证且动作可能已经发送时暂停该动作、保留现场且不盲目重发。其他未选中机器离线、锁定或能力异常不影响本任务。
