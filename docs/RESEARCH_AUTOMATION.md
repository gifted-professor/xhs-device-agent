...
Research 不再有第二个包装器、第二套锁、第二次确认或独立执行入口。
...
仓库没有永久的四设备限制。任务提出设备和 `maxParallel`，当前人工接受的能力档案决定本次实现是否已经验证到该规模；不满足时在 App 导航前拒绝，不会缩小任务或改用旧链路。
...
- 登录、验证码、身份验证、权限、订单/支付、私信/联系人、平台风控、私密页面或设备/目标漂移立即停止；
- 不会关闭、接受、绕过或重试穿过强制停止页面。
