...
- 单设备串行执行；
- 每次运行必须使用新的任务 ID；
- 设备别名必须来自当前 `xhs.cmd device list`，不得照抄文档占位符。
...
runbook 中的 `device-01` 是示例占位符，但当前本地配置没有该别名。首次调用在任务目录创建和任何互动发送之前，以 `DeviceAlias must resolve to exactly one local device` 安全失败。随后通过 `xhs.cmd device list` 核对，改用唯一在线映射 `device-04`，同一规格完整通过。

模板不固化设备别名，只固化已经验收的内容数量、互动位置和视频停留参数，从而避免把某台机器的配置复制到另一台机器。
