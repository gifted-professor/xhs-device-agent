...
```bash
# 整表标准草稿（校准字段全开 + 终点存草稿）
node scripts/xianyu-operator.mjs --serial 1511f78c --transport gateway publish-dry-run \
  --description "..." \
...
# 仅存草稿（已在编辑页）
node scripts/xianyu-operator.mjs --serial 1511f78c --transport gateway save-draft-dry-run
```
