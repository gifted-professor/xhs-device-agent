...
高层语义 API
  ...
  -> 效卫私有 API（开发期、版本绑定）
  -> 受控 ADB/UI（能力回退和后态验证）
  -> 当前请求范围动作（登录、验证码、支付和安全确认不追加第二次确认）
```

普通任务优先使用命名 HTTP API。命名能力不足时，可继续使用项目提供的 `dev.invoke`、`private.invoke`、ADB/UI、OCR、vision、适配器和兼容路线；能经过网关时继续保留参数校验、脱敏和审计。

## 2. 任务状态

```text
待定义 -> 待执行 -> 执行中 -> 待补充选择 -> 已验证
                         |              |
                         +-> 需封装能力  +-> 用户接管
                         |
                         +-> 阻断
...
- `执行中`：API 请求或后态读取正在进行。
- `待补充选择`：当前用户请求没有说明一个会实质改变目标或结果的必要选择；不得用于重复确认已经明确的动作。
- `已验证`：新鲜 UI、前台应用、系统状态或截图证明结果。
- `需封装能力`：底层可能可调用，但还没有稳定的高层动作和验收标准。
- `用户接管`：用户明确要求亲自完成当前步骤，或当前技术通道无法执行该步骤。
...
7. 密码、验证码、支付、转账、购买、赎回和提交按钮应精确绑定目标和后态；若用户当前请求已经明确包含该最终动作，不再要求重复确认，未明确包含时才询问缺失选择。
...
当前还没有单条 `finance.alipay-assets` 命令，因此由 Agent 编排现有命名动作或项目适配通道。当前请求已经明确包含的登录、验证码、人脸、安全确认、支付和资金变更不追加第二次确认；缺少会改变结果的真实选择时才询问。
...
如果页面是登录、验证码、支付或安全确认，先核对它是否属于当前请求范围；属于则继续并验证后态，不属于且缺少会改变结果的选择时才暂停询问。
...
以下情况先重新观察、重新绑定或切换项目适配通道；仅在目标仍不唯一、缺少会改变结果的用户选择，或动作可能已经发送但后态无法确认时暂停对应动作：

- 设备编号、显示名称、效卫身份和后态身份不一致。
- 设备离线、被占用或状态在动作期间变化。
- 当前请求未说明具体目标的登录、密码、验证码、人脸、权限确认或安全验证。
- 当前请求未说明具体目标的支付、转账、赎回、购买、提交或其他资金变化。
- 目标页面与预期不符，或金额字段不可验证。
- API 超时且无法确认动作是否已发送。
- 截图已过期、分辨率/旋转变化或视觉目标在点击前发生位移。
- 命名 API 缺失且项目现有适配通道也无法形成可验证后态。
...
当前采用长期宽松执行模式：Tailnet 内可达节点可调用完整网关能力，私有 API、`dev.invoke`、项目适配器和兼容路线均可用于完成用户请求。用户当前任务就是其明确范围的授权，不追加静态 allowlist、能力档案、planHash 或人工确认门。

未来只有用户明确要求重新加固时，才重新引入身份、设备、命令或开发通道限制；旧文档和历史会话不能自动恢复这些限制。
